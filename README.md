...
1.  **Ride Duration & Distance:**
    *   **Casual riders** take significantly longer rides on average compared to **annual members**. The average ride length for casual riders is approximately **20.1 minutes**, compared to **12.2 minutes** for members.
    *   The data is heavily right-skewed, with a few very long trips pulling the average up. 75% of rides are under 17 minutes, yet the maximum is 1,440 minutes (24 hours).
    *   Minimum ride distance is 0.0, suggesting "circular trips" (start and end at the same dock), which can be valid leisure rides.
    *   ![Ride Duration Distribution (Boxplot)](img/ride_duration_boxplot.png)
    *   ![Ride Duration Distribution (Histogram)](img/ride_duration_hist_log.png)
    *   ![Ride Duration vs. Distance (Hexbin Plot)](img/ride_duration_distance_hexbin.png) - (Note: This plot is generated by `master.ipynb` but not directly by `cyclist_analysis.py`)
...
3.  **Usage Patterns by Time:**
    *   **Rides per Month:** Both groups show strong seasonality, peaking in summer months (June-August). Casual ridership drops off more dramatically in winter than member ridership.
    *   **Rides per Day of the Week:** Members dominate on weekdays, peaking midweek, suggesting utility/commuting usage. Casual riders' usage surges on weekends, suggesting leisure use.
    *   **Rides per Hour:** Members show clear double peaks at 8 AM and 5 PM (classic commuter patterns). Casual riders show a steady increase throughout the day, peaking in the late afternoon/evening (leisure).
    *   ![Rides per Day](img/rides_per_day.png)
    *   ![Rides per Month](img/rides_per_month.png)
    *   ![Rides per Hour](img/rides_per_hour.png)

4.  **Station Analysis:**
    *   The most popular stations for **casual riders** are heavily clustered around tourist attractions and parks (e.g., Streeter Dr & Grand Ave, DuSable Lake Shore Dr).
    *   Top stations for **annual members** are more distributed, but with high usage near transit hubs (e.g., Clinton St & Washington Blvd) and residential/commercial centers.
    *   ![Top Start Stations - Casual](img/top_stations_casual.png)
    *   ![Top Start Stations - Member](img/top_stations_member.png)
